...
module.exports = function formatBudget(data) {
  ...
  var rows = combineRows([
    majorDivider,
    makeRows(data.proportional.accounts,toPercent),
    minorDividor,
    makeRow('Total', data.proportional.total, toPercent),
    majorDivider,
    makeRows(data.fixed.accounts),
    minorDividor,
    makeRow('Total', data.fixed.total),
    majorDivider
  ]);
